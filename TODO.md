...
## Claude generated
 ...
 - [ ] Cancel still doesn't interrupt `waitForIndex` (≤30s) or `listFolder` — they're separate blocking JNI calls not wired to the connect context, so if you cancel after the TLS handshake but before the index settles, the coroutine keeps running invisibly in the background and only discards the client once those return. Thread a cancellation signal into the model's index wait to make cancel fully prompt.
